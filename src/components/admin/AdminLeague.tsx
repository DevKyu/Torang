...
const AdminLeague = () => {
  ...
  const [playerDropdowns, setPlayerDropdowns] = useState<Record<string, [string, string][]>>({});
  ...
  const lookupByName = (teamKey: 'team1' | 'team2', idx: number) => {
    ...
    if (matches.length === 0) {
      toast.error('검색 결과가 없습니다', { position: 'top-center', duration: 1500 });
      return;
    }
    if (matches.length === 1) {
    ...
    setPlayerDropdowns((prev) => ({ ...prev, [`${teamKey}_${idx}`]: matches }));
  };

  const selectFromDropdown = (teamKey: 'team1' | 'team2', idx: number, empId: string, name: string) => {
    setEditing((prev) => {
      if (!prev) return prev;
      const team = [...prev[teamKey]];
      team[idx] = { ...team[idx], empId, name };
  ...
  const updatePlayer = (teamKey: 'team1' | 'team2', idx: number, field: keyof PlayerEntry, value: string) => {
    setEditing((prev) => {
      if (!prev) return prev;
      const team = [...prev[teamKey]];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      toast(`✅ ${editing.groupId}조 저장되었습니다`, {
        position: 'top-center',
        duration: 2000,
        style: { backgroundColor: '#f0fdf4', color: '#065f46', borderRadius: '10px', fontSize: '0.875rem' },
      });
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast(`🗑️ ${editing.groupId}조 삭제되었습니다`, {
        position: 'top-center',
        duration: 2000,
        style: { backgroundColor: '#fef2f2', color: '#b91c1c', borderRadius: '10px', fontSize: '0.875rem' },
      });
      await loadGroups(yyyymm);
      setEditing(null);
      setConfirmDelete(false);
          ...
          {playerDropdowns[`${teamKey}_${idx}`]?.length > 0 && (
            <NameDropdown>
              {playerDropdowns[`${teamKey}_${idx}`].map(([eid, n]) => (
                <NameDropdownItem key={eid} onClick={() => selectFromDropdown(teamKey, idx, eid, n)}>
                  {n}
                  <span>{eid}</span>
                </NameDropdownItem>
              ))}
  ...
  return (
    <AdminLayout title="정기전 관리">
              ...
              Object.entries(groups)
                ...
                .map(([groupId, g]) => (
                  <GroupRow key={groupId} onClick={() => openEdit(groupId)}>
                    ...
                    <span>
                      {g.winner === 'team1' ? '1팀 승' : g.winner === 'team2' ? '2팀 승' : '무승부'}
                    </span>
                    <GroupDate>
                      {g.date
      ...
      {editing && (
        <FormSection>
          <FormTitle>
            {groups[editing.groupId] ? `${editing.groupId}조 편집` : `${editing.groupId}조 추가`}
          </FormTitle>
          ...
          <WinnerRow>
            {(['team1', 'team2', 'draw'] as const).map((w) => (
              <WinnerBtn
                ...
                onClick={() => setEditing((prev) => prev && { ...prev, winner: w })}
              >
          ...
          <SaveRow>
            ...
            {!confirmDelete && (
              <CancelBtn onClick={() => { setEditing(null); setConfirmDelete(false); }}>
                취소
              </CancelBtn>
            )}
          </SaveRow>
