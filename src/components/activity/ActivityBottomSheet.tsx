        ...
        >
          ...
          >
            ...
            <Content>
              ...
              {item.type === 'match' && item.teams && (() => {
                const myStatus = item.delta > 0 ? 'winner' : item.delta < 0 ? 'loser' : 'neutral';
                const oppStatus = item.delta > 0 ? 'loser' : item.delta < 0 ? 'winner' : 'neutral';
                return (
                  <>
                    <TeamsRow>
                      <TeamBlock status={myStatus}>
                        <TeamLabelRow>
                          <TeamLabel status={myStatus}>
                            {item.teams.my.length === 1 ? item.teams.my[0] : '우리팀'}
                          </TeamLabel>
                          {item.scores && (
                            <TeamScoreNum status={myStatus}>{item.scores.my}</TeamScoreNum>
                          )}
                        </TeamLabelRow>
                      ...
                      <TeamBlock status={oppStatus}>
                        <TeamLabelRow>
                          <TeamLabel status={oppStatus}>
                            {item.teams.opponent.length === 1 ? item.teams.opponent[0] : '상대팀'}
                          </TeamLabel>
                          {item.scores && (
                            <TeamScoreNum status={oppStatus}>{item.scores.opponent}</TeamScoreNum>
                          )}
                        </TeamLabelRow>
              ...
              {item.type === 'league' && (() => {
                const myStatus = item.result === 'win' ? 'winner' : item.result === 'lose' ? 'loser' : 'neutral';
                const oppStatus = item.result === 'win' ? 'loser' : item.result === 'lose' ? 'winner' : 'neutral';
                return (
                  <>
                    <TeamsRow>
                      <TeamBlock status={myStatus}>
                        <TeamLabelRow>
                          ...
                          <TeamScoreNum status={myStatus}>{item.myTotalScore}</TeamScoreNum>
                        </TeamLabelRow>
                      ...
                      <TeamBlock status={oppStatus}>
                        <TeamLabelRow>
                          ...
                          <TeamScoreNum status={oppStatus}>{item.opponentTotalScore}</TeamScoreNum>
                        </TeamLabelRow>
                ...
                (item.stats ? (
                  <StatsGrid>
                    ...
                    <StatItem kind="comments">
                      <StatEmoji>💬</StatEmoji>
                      <StatValue kind="comments">{item.stats.comments}</StatValue>
                      <StatLabel>댓글</StatLabel>
                    </StatItem>
                    <StatItem kind="achievements">
                      <StatEmoji>🏆</StatEmoji>
                      <StatValue kind="achievements">{item.stats.achievements}</StatValue>
